    ...
    mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'data-theme') {
            ...
            if (tempChart && humidityChart) {
                const forecastData = tempChart.data.labels.map((label, index) => ({
                    day_name: label,
                    temp_avg: tempChart.data.datasets[0].data[index],
                    temp_max: tempChart.data.datasets[1].data[index],
                    temp_min: tempChart.data.datasets[2].data[index],
                    humidity: humidityChart.data.datasets[0].data[index],
                    rainfall: humidityChart.data.datasets[1].data[index]
                }));

                initializeCharts(forecastData);
            }
        }
    });
});
